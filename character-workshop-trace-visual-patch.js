// 生字練習工坊視覺修補：仿寫練習改成明顯彩色範字，並在第一筆起點加紅色粗體提示點。
(function () {
  ...
  function installStyle() {
    ...
    style.id = STYLE_ID;
    style.textContent = `
      #cw-model { opacity: 1 !important; }
      #cw-model .cw-svg { opacity: 1 !important; }
      #cw-model .cw-trace-color-path { opacity: .48 !important; }
      .cw-start-dot { filter: drop-shadow(0 2px 4px rgba(220, 38, 38, .55)); }
      .cw-start-ring { animation: cwStartPulse 1.15s ease-in-out infinite; transform-origin: center; }
      @keyframes cwStartPulse { 0%,100%{opacity:.20} 50%{opacity:.70} }
      #cw-start-tip {
  ...
  function coloredTraceSvg(data, ch) {
    ...
    const paths = strokes.map((d, idx) => {
      ...
      return `<path class="cw-trace-color-path" d="${d}" fill="${color}"/>`;
    }).join('');

    const firstPoint = medians?.[0]?.[0];
    const startDot = firstPoint
      ? `<circle class="cw-start-ring" cx="${firstPoint[0]}" cy="${firstPoint[1]}" r="42" fill="#ef4444" opacity=".25"/>
         <circle class="cw-start-dot" cx="${firstPoint[0]}" cy="${firstPoint[1]}" r="25" fill="#ef4444" stroke="#ffffff" stroke-width="8"/>`
      : '';
  ...
  async function applyTraceVisual() {
    ...
    const key = ch + '|' + (el('cw-split-input')?.value || '') + '|colored-v2';
