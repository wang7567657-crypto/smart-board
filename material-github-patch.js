...
// 重要：練習工坊只載入一個正式版 character-workshop-click-fix.js，避免舊補丁互相干擾。
(function () {
  ...
  function getMaterialData() {
    ...
    return {
      ...
      primaryTsv: getEl('primary-tsv-input')?.value || '',
      extraTsv: getEl('extra-tsv-input')?.value || '',
  ...
  async function saveMaterialToGithub() {
    ...
    try {
      const result = await window.uploadToGithub(meta.folderName, meta.fileName, JSON.stringify(data, null, 2), true);
      if (result && result.success) setStatus(`✅ 已儲存：${meta.folderName}/${meta.fileName}`, 'success');
      else {
  ...
  async function loadMaterialFromGithub() {
    ...
    try {
      const res = await fetch(getRawMaterialUrl(meta), { cache: 'no-store' });
  ...
  function copyMaterialJson() {
    const content = JSON.stringify(getMaterialData(), null, 2);
    navigator.clipboard?.writeText(content)
      .then(() => setStatus('已複製 JSON，可手動貼到 GitHub。', 'success'))
      .catch(() => { console.log(content); setStatus('無法自動複製，已輸出到 console。', 'warn'); });
  }
  ...
  function insertMaterialPanel() {
    ...
    box.innerHTML = `
      ...
      <button id="btn-save-material-github" class="w-full mb-2 bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-2 px-4 rounded-lg shadow-sm text-sm transition-transform hover:-translate-y-0.5">儲存本課教材到 GitHub</button>
      <button id="btn-load-material-github" class="w-full mb-2 bg-sky-500 hover:bg-sky-600 text-white font-bold py-2 px-4 rounded-lg shadow-sm text-sm transition-transform hover:-translate-y-0.5">從 GitHub 載入本課教材</button>
      <button id="btn-copy-material-json" class="w-full mb-2 bg-slate-500 hover:bg-slate-600 text-white font-bold py-2 px-4 rounded-lg shadow-sm text-sm transition-transform hover:-translate-y-0.5">複製教材 JSON</button>
  ...
  function cleanupOldWorkshopScripts() {
    ['tab-workshop', 'tab-workshop-template'].forEach(id => document.getElementById(id)?.remove());
  }
  ...
  function applyPatch() {
    ...
    loadExternalPatch('character-workshop-final-script', './character-workshop-click-fix.js');
    cleanupOldWorkshopScripts();
    insertMaterialPanel();
  }
  ...
  setInterval(cleanupOldWorkshopScripts, 1500);
})();