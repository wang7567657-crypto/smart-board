...
(function () {
  ...
  function installStyle() {
    ...
    style.id = STYLE_ID;
    style.textContent = `
      .ws-sheet{font-family:'Noto Sans TC','Microsoft JhengHei',sans-serif;color:#111827;width:210mm;min-height:297mm;padding:12mm;background:#fff;box-sizing:border-box;}
      ...
      @media print{body *{visibility:hidden!important;}#${ROOT_ID},#${ROOT_ID} *{visibility:visible!important;}#${ROOT_ID}{position:absolute;left:0;top:0;width:100%;}.ws-sheet{width:210mm;min-height:297mm;}.ws-no-print{display:none!important;}}
    `;
  ...
  function root() {
    let r = el(ROOT_ID);
    if (!r) { r = document.createElement('div'); r.id = ROOT_ID; r.style.display = 'none'; document.body.appendChild(r); }
    return r;
  }
  ...
  function strokeWorksheet() {
    ...
    show(`<div class="ws-sheet">${header('筆順作業單')}<div class="ws-section">一、看清楚生字後，照著筆順練習書寫。</div><table class="ws-table"><thead><tr><th>題號</th><th>生字</th><th>練習1</th><th>練習2</th><th>練習3</th><th>練習4</th></tr></thead><tbody>${rows}</tbody></table><div class="ws-no-print" style="text-align:center;color:#64748b;font-size:12px;">列印完成後可關閉列印視窗。</div></div>`);
  }
  ...
  function dictationWorksheet() {
    ...
    show(`<div class="ws-sheet">${header('聽寫作業單')}<div class="ws-section">一、聽老師唸注音或語詞，寫出正確的國字。</div><table class="ws-table"><thead><tr><th>題號</th><th>生字注音</th><th>寫出生字</th><th>語詞注音</th><th>寫出語詞</th></tr></thead><tbody>${body}</tbody></table></div>`);
  }
  ...
  function correctionWorksheet() {
    ...
    show(`<div class="ws-sheet">${header('線上改錯題')}<div class="ws-section">一、讀一讀句子，找出需要注意或可能寫錯的字詞，並寫出改正答案。</div><table class="ws-table"><thead><tr><th>題號</th><th>句子</th><th>錯字</th><th>改正</th></tr></thead><tbody>${body}</tbody></table></div>`);
  }
